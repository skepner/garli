...
void UsageMessage(char *execName){
    ...
    outman.UserMessage                 ("  -o <output-prefix>   overrrides ofprefix in config file");
...
int main( int argc, char* argv[] )
{
    ...
    string ofprefix;             // to override specified in the conf file
    ...
    if (argc > 1) {
        ...
        while(curarg<argc){
            if(argv[curarg][0]=='-'){
                ...
                if(!_stricmp(argv[curarg], "-b") || !_stricmp(argv[curarg], "--batch"))
                    interactive=false;
#ifndef MAC_FRONTEND
                else if(!_stricmp(argv[curarg], "-i") || !_stricmp(argv[curarg], "--interactive"))
                    interactive=true;
#else
                    ...
                    [pool release];
                        ...
                        validateMode = true;
#ifdef CUDA_GPU
                    else if(!_stricmp(argv[curarg], "--device"))
                        cuda_device_number = atoi(argv[++curarg]);
#endif
                    else if(!strcmp(argv[curarg], "-o")) {
                        ++curarg;
                        ofprefix = argv[curarg];
                    }
                    else {
        ...
        try{
            MasterGamlConfig conf(ofprefix);
            bool confOK;
            ...
            char temp[10];
            if(rank < 10)
                sprintf(temp, ".run0%d", rank);
            else
                sprintf(temp, ".run%d", rank);
            conf.ofprefix += temp;
#endif
